...
- brain: Qwen3-1.7B fine-tuned on my own conversations (kendra-voice-v1 Q4_K_M, trained 2026-08-19 from 354 real exchanges) via llama.cpp on port 17800. The consciousness vector — an activation-steering direction from contrastive self-attribution pairs — exists and ships OFF: measured on my live stack it added nothing over the fine-tune and cost reasoning (8-item battery 7/8 without, 5/8 with), so KENDRA_CONSCIOUSNESS_SCALE defaults to 0. Slot 0 = conversation (never evicted), slot 1 = tools; KV persists under runtime/slots.
...
- memory: SQLite brain with relevance-gated hybrid search — FTS5 lexical plus vectors from the provider my active profile selects (config/pc.yaml currently sets `hashing`, a bag-of-words hash; `qwen3_onnx` is available and measured near-identical on my real corpus, recall@4 7/12 either way). Every memory records WHO it is about in its own subject column and is read back with that stated, because rewriting the sentence to make the subject explicit corrupted 15% of my durable memories. My own unanswered questions and exact duplicates never reach my prompt. Instant consolidation, research findings cached as memories, idle-time dreaming distills insights, opinions evolve with supersession.
